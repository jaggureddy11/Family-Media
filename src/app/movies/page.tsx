...
export default function MoviesPage() {
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-6 sm:gap-8 pb-16">
        ...
        <div className="flex flex-wrap items-center gap-3 sm:gap-4">
          <button
            ...
            className={`kutumbam-focus min-h-[64px] sm:min-h-[72px] px-6 sm:px-8 rounded-2xl font-bold text-[var(--text-btn)] border-4 transition-all duration-150 active:scale-95 ${
              activeFilter === "ALL"
          ...
          <button
            ...
            className={`kutumbam-focus min-h-[64px] sm:min-h-[72px] px-6 sm:px-8 rounded-2xl font-bold text-[var(--text-btn)] border-4 flex items-center gap-3 transition-all duration-150 active:scale-95 ${
          ...
          {searchQuery && (
            <button
              ...
              className="min-h-[64px] px-5 bg-red-950/80 border-2 border-red-500 text-red-200 rounded-2xl text-[var(--text-body)] font-bold flex items-center gap-2 hover:bg-red-900"
            >
              <X className="w-6 h-6" />
        ...
        {/* Poster Grid: 1 per row at Huge, 2 on phone (390px), 3 on TV (1920px) */}
        {loading ? (
          <div className="py-20 text-center text-white text-[var(--text-body)] font-medium">
            <Bi k="gettingReady" />
          </div>
        ) : movies.length === 0 ? (
          <div className="py-20 px-6 text-center bg-[var(--bg-surface)] border-4 border-[var(--border-subtle)] rounded-3xl space-y-6 max-w-xl mx-auto">
            <p className="text-[var(--text-heading)] font-bold text-slate-300">
      ...
      {showSearchModal && (
        <div
          className="fixed inset-0 z-50 bg-black/95 flex flex-col p-4 sm:p-10 justify-start items-center"
          role="dialog"
          aria-modal="true"
        >
          <div className="w-full max-w-3xl flex justify-between items-center mb-8 border-b-4 border-slate-800 pb-4">
          ...
          <div className="w-full max-w-3xl space-y-6">
            <div className="relative flex items-center">
              <input
                ...
                className="w-full min-h-[84px] px-6 pr-24 bg-[var(--bg-surface-elevated)] border-4 border-white text-white rounded-3xl text-[var(--text-btn)] font-medium placeholder-slate-400 focus:ring-6 focus:ring-yellow-400 outline-none"
              />
              <button
                type="button"
                onClick={startVoiceSearch}
                aria-label="Voice Search · వాయిస్ సెర్చ్"
                className={`absolute right-4 p-4 rounded-2xl border-2 transition-all ${
                  isListening
            ...
            {isListening && (
              <div className="p-6 bg-red-950/80 border-4 border-red-500 rounded-2xl text-center text-red-200 text-[var(--text-body)] font-bold animate-pulse">
                <Bi k="listening" /> · <Bi k="speakNow" />
              </div>
            )}

            {voiceError && (
              <div className="p-4 bg-slate-800 border-2 border-slate-600 rounded-2xl text-center text-slate-300 text-[var(--text-body)]">
        ...
        >
          <div className="bg-[var(--bg-surface-elevated)] border-6 border-[var(--accent)] rounded-3xl p-6 sm:p-12 max-w-2xl w-full flex flex-col items-center text-center gap-8 shadow-2xl">
            <h2 className="text-[var(--text-heading)] font-bold text-white">
              {resumeMovie.titleEn}
              <div className="text-[1.2em] text-yellow-300 font-sans mt-2">
                {resumeMovie.titleTe}
              </div>
            </h2>
            ...
            <div className="flex flex-col w-full gap-5">
              ...
              <button
                ...
                className="kutumbam-focus min-h-[88px] px-8 py-5 bg-[var(--accent)] hover:bg-yellow-300 text-black border-4 border-white rounded-3xl flex items-center justify-center gap-4 text-[var(--text-btn)] font-extrabold shadow-lg active:scale-95"
                data-nav-item="true"
              >
                <Play className="w-10 h-10 fill-current shrink-0" />
                <Bi
                  text={{
                  ...
                  layout="stacked"
                  enClassName="text-[0.9em]"
                  teClassName="text-[1.1em] font-bold"
                />
              </button>
              ...
              <button
                ...
                className="kutumbam-focus min-h-[88px] px-8 py-5 bg-slate-800 hover:bg-slate-700 text-white border-4 border-slate-500 hover:border-white rounded-3xl flex items-center justify-center gap-4 text-[var(--text-btn)] font-bold shadow-lg active:scale-95"
                data-nav-item="true"
              >
