...
| `/api/admin/system-status` | `GET` | **Admin Only** | [`src/app/api/admin/system-status/route.ts`](file:////Users/apple/Desktop/PROJECTS/Family media/src/app/api/admin/system-status/route.ts) | Admin system status (storage provider & database connection verification) |
